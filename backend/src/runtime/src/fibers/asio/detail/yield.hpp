...
struct yield_completion {
    ...
    using mutex_t = fibers::detail::spinlock;
    using lock_t = std::unique_lock<mutex_t>;
    using ptr_t = boost::intrusive_ptr<yield_completion>;
...
public:
    explicit yield_handler_base(yield_t const &y)
...
void asio_handler_invoke(Fn &&fn, yield_handler<T> * /*unused*/) {
    fn();
}
