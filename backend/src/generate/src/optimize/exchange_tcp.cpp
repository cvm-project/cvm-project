...
void ExchangeTcp::Run(DAG *const dag, const std::string & /*config*/) const {
    for (auto *const pop_candidate : dag->operators()) {
        ...
        for (auto *const op : partitioned_exchange_operators) {
            ...
            auto *const exchange_op = new DAGExchangeTcp();
            ...
            dag->AddFlow(row_scan_op, exchange_op);
            dag->AddFlow(exchange_op, column_scan_op);
