...
/*  0 */ _CFFI_OP(_CFFI_OP_FUNCTION, 1), // int *()(int *, int *, int, int, int *)
...
{
  return intersect(x0, x1, x2, x3, x4);
}
#ifndef PYPY_VERSION
static PyObject *
_cffi_f_intersect(PyObject *self, PyObject *args)
{
  int * x0;
  int * x1;
  int x2;
  ...
  if (!PyArg_UnpackTuple(args, "intersect", 5, 5, &arg0, &arg1, &arg2, &arg3, &arg4))
    return NULL;
  ...
  Py_BEGIN_ALLOW_THREADS
  _cffi_restore_errno();
  { result = intersect(x0, x1, x2, x3, x4); }
  _cffi_save_errno();
  Py_END_ALLOW_THREADS
...
#else
#  define _cffi_f_intersect _cffi_d_intersect
#endif
...
#ifdef PYPY_VERSION
PyMODINIT_FUNC
_cffi_pypyinit_operators(const void *p[])
{
...
#  ifdef _MSC_VER
     PyMODINIT_FUNC
#  if PY_MAJOR_VERSION >= 3
     PyInit_operators(void) { return NULL; }
#  else
     initoperators(void) { }
#  endif
#  endif
...
{
  return _cffi_init("operators", 0x2601, &_cffi_type_context);
}
#else
PyMODINIT_FUNC
initoperators(void)
{
  _cffi_init("operators", 0x2601, &_cffi_type_context);
}
#endif
