...
size_t join(tuple_2 *array1, tuple_3 *array2) {
...
//    t2 = high_resolution_clock::now();
    ...
    //std::cout << (*res)[rand() % res->size()].v0 << " " << (*res)[rand() % res->size()].v1 << " "
    //        << (*res)[rand() % res->size()].v2 << std::endl;
//      std::cout << res << std::endl;
//    duration = duration_cast<milliseconds>(t2 - t1).count();
//    std::cout << duration << " join\n";
//    free(array1);
//    free(array2);
    return 0;
}
