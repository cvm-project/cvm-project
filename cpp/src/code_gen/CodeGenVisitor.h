...
private:
    ...
    void emitLLVMFunctionWrapper(DAGOperator *op, const std::string &opName,
        ...
        if (returnsBool) {
            llvmFuncDecls.push_back("bool " + llvmFuncName + "(" +
                                    flatInputType + ");");
        } else {
    ...
    void emitFuncEnd(const std::string &action) {
        ...
        if (action == "count") {
            ...
            appendLineBodyNoCol("while (auto res = " +
                                getCurrentOperatorName() + ".next()) {");
            tabInd++;
            ...
            emitComment("reduce");
            appendLineBody(getCurrentOperatorName() + ".open()");
            appendLineBody("auto res = " + getCurrentOperatorName() +
                           ".next()");
            appendLineBody(getCurrentOperatorName() + ".close()");
            //            appendLineBodyNoCol("TOCK1");
    ...
    std::string writeTupleDefs(const std::string &action) {
        ...
        if (action == "collect") {
            ...
            resultWrapper =
                    "typedef " + getCurrentTupleName() + "* result_type;\n";
        }
