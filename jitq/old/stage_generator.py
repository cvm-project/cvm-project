from jitq.old.ast_generator import ROOT_HASH_TABLE, RESULT_LIST, ASTGenerator, FUNCTION_NAME, PARAMETER_LIST, \
    PARAMETER_HASH_TABLE
from jitq.old.stage_ast import StageAst
import jitq.rdd
from jitq.old.ast_shortcuts import *
...
def is_stage_breaker(rdd):
    return isinstance(rdd, jitq.rdd.ShuffleRDD)
...
def append_breaker_ast(source_breaker, cur_stage):
    ...
    :return:
    """
    if isinstance(source_breaker, jitq.rdd.Join):
...
def generate_breaker_stage(node):
    ...
    :return:
    """
    if isinstance(node, jitq.rdd.Join):
        stage_ast = StageAst()
        body_ast = []
