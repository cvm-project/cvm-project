...
class TestReduce(unittest.TestCase):
    def test_reduce_sum(self):
        bc = JitqContext()
        inpt = range(0, 10)
        d = bc.collection(inpt).reduce(lambda t1, t2: t1 + t2)
        self.assertEqual(d, sum(inpt))

    def test_reduce_tuple(self):
        bc = JitqContext()
        inpt = [(r, r * 10) for r in range(0, 10)]
        res = bc.collection(inpt).reduce(lambda t1, t2: (t1[0] + t2[0], t1[1] + t2[1]))
        res = tuple(res)
        truth = (sum(map(lambda t: t[0], inpt)), sum(map(lambda t: t[1], inpt)))
...
class TestReduceByKey(unittest.TestCase):
    def test_reduce_by_key(self):
        ...
        d = bc.collection(inpt).reduce_by_key(lambda t1, t2: t1 + t2).collect()
        truth = {(0, 2), (1, 3)}
